 ...
 *
 */
void ADI3DToFFloorDetector::processOutputAbort()
{
 ...
 *
 * @param new_output_node : Pointer to the output node o be published
 */
void ADI3DToFFloorDetector::floorDetectorIOThreadPushOutputNode(ADI3DToFFloorDetectorOutputInfo* new_output_node)
{
...
void ADI3DToFFloorDetector::processOutput()
{
  ...
  while ((!process_output_thread_abort_) || (output_queue_size > 0))
  {
    ...
    if (output_queue_size > 0)
    {
      ...
      output_thread_mtx_.lock();
      ADI3DToFFloorDetectorOutputInfo* new_frame = (ADI3DToFFloorDetectorOutputInfo*)output_node_queue_.front();
