 ...
 */
void OutputSensorFile::write(int frame_number, const cv::Mat& m_out_visualization_image, bool floor_detection_status,
                             int ransac_iterations, int noise_count, float ransac_time_out)
{
  if (csv_enabled_)
  {
    writeOutputCsvFile(frame_number, floor_detection_status, ransac_iterations, noise_count, ransac_time_out);
  }
 ...
 */
void OutputSensorFile::writeOutputCsvFile(int frame_number, bool floor_detection_status, int ransac_iterations,
                                          int noise_count, float ransac_time_ms_out)
{
  if (output_csv_file_.is_open())
  {
    output_csv_file_ << frame_number << "," << floor_detection_status << "," << ransac_iterations << "," << noise_count
                     << "," << ransac_time_ms_out << std::endl;
  }
}
