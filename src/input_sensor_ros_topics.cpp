...
typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image> sync_depth_ab;
                                     ...
                                     std::string /*config_file_name*/, std::string input_sensor_ip)
{
  ...
  // Subscribe to camera-info, depth, ab and point-cloud topics
  ...
  // We need to sync the depth and ab topics
  static message_filters::Synchronizer<sync_depth_ab> sync_depth_and_ab(sync_depth_ab(MAX_QUEUE_SIZE_FOR_TIME_SYNC));
  sync_depth_and_ab.connectInput(depth_image_subscriber_, ab_image_subscriber_);
  sync_depth_and_ab.registerCallback(boost::bind(&InputSensorRosTopic::syncDepthandIr, this, _1, _2));
...
/**
 * @brief synchronizes depth and AB images
 *
 * @param depth_image depth image pointer
 * @param ab_image ab image pointer
 */
 ...
 * @param ab_frame pointer to read ab frame
